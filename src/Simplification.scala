...
object Simplification
{
  ...
  def org( e:Exp ) : Exp = e match
  {
    ...
    case Dot(VMul(u,Vec(s1)),Cross(Vec(s2),Vec(s3))) =>
        if       ((s1 < s2) && (s2 < s3)) {Dot(Vec(s1),Cross(VMul(u,Vec(s2)),Vec(s3)))}
        else if  ((s1 < s3) && (s3 < s2)) {Dot(Vec(s1),Cross(VMul(u,Vec(s2)),Vec(s3)))}
        else if  ((s2 < s1) && (s1 < s3)) {Dot(Vec(s2),Cross(VMul(u,Vec(s3)),Vec(s1)))}
        else if  ((s2 < s3) && (s3 < s1)) {Dot(Vec(s2),Cross(VMul(u,Vec(s3)),Vec(s1)))}
        else if  ((s3 < s1) && (s1 < s2)) {Dot(Vec(s3),Cross(VMul(u,Vec(s1)),Vec(s2)))}
        else                              {Dot(Vec(s3),Cross(VMul(u,Vec(s1)),Vec(s2)))}

    case Dot(Vec(s1),Vec(s2)) => if (s1 < s2) {Dot(Vec(s1),Vec(s2))} else {Dot(Vec(s2),Vec(s1))}
    case Dot(Vec(s1),Cross(Vec(s2),Vec(s3))) =>
        if       ((s1 < s2) && (s2 < s3)) {Dot(Vec(s1),Cross(Vec(s2),Vec(s3)))}
        else if  ((s1 < s3) && (s3 < s2)) {Dot(Vec(s1),Cross(Vec(s2),Vec(s3)))}
        else if  ((s2 < s1) && (s1 < s3)) {Dot(Vec(s2),Cross(Vec(s3),Vec(s1)))}
        else if  ((s2 < s3) && (s3 < s1)) {Dot(Vec(s2),Cross(Vec(s3),Vec(s1)))}
        else if  ((s3 < s1) && (s1 < s2)) {Dot(Vec(s3),Cross(Vec(s1),Vec(s2)))}
        else                              {Dot(Vec(s3),Cross(Vec(s1),Vec(s2)))}
    ...
    case Mul(u,Dot(a,b)) =>
        ...
        else {Mul(u,org(Dot(a,b)))}
    case Mul(Mul(u,Dot(a,b)),v) => org(Mul(u,Mul(v,Dot(a,b))))
    
    case Mul(Var(s),Var(t)) =>
        if (s <= t) {Mul(Var(s),Var(t))}
        else {Mul(Var(t),Var(s))}
      
    case Mul(Var(s),Mul(Var(t),u)) =>
        if (org(Mul(Var(t),u)) != Mul(Var(t),u)) {org(Mul(Var(s),org(Mul(Var(t),u))))}
        else if (s <= t) {Mul(Var(s),Mul(Var(t),u))}
        else {org(Mul(Var(t),Mul(Var(s),u)))}
      
    case Mul(u,Var(s)) => org(Mul(Var(s),u))
    
    case Mul(Mul(Var(s),u),Mul(Var(t),v)) =>
        if (s <= t) {org(Mul(Var(s),Mul(Var(t),Mul(u,v))))}
        else {org(Mul(Var(t),Mul(Var(s),Mul(u,v))))}
  ...
  def removeOneCoefs ( e:Exp ) : Exp = e match
  {
    ...
    case Mul(u,v) =>
        if (u == Num(1))       {removeOneCoefs(v)}
        else if (v == Num(1))  {removeOneCoefs(u)}
        else if ((removeOneCoefs(u) != u) || ((removeOneCoefs(v) != v))) {removeOneCoefs(Mul(removeOneCoefs(u), removeOneCoefs(v)))}
        else {Mul(removeOneCoefs(u), removeOneCoefs(v))}
  ...
  def findZeros( e:Exp ) : Boolean = e match
  {
     ...
     case Num(d) => if (d==0) {return true} else {return false}
     case u:Exp => false
  }
  ...
  def simp ( e:Exp ) : Exp = e match
  {
    ...
    case Add(Num(d),u) => if (d == 0) {simp(u)} else {Add(Num(d),simp(u))}
    ...
    case Mul(u,v) => if (findZeros(Mul(u,v))) {Num(0)} else {Mul(Num(combine(Mul(u,v))),terms(Mul(u,v)))}
    case Dot(a,b) => Dot(a,b)
    case u:Exp => u
    ...
    case Dot(a,Cross(b,c)) => if ((a == b) || (a == c) || (b == c)) {Num(0)} else {Dot(a,Cross(b,c))}
    case Dot(a:VExp, b:VExp) =>
        if      ((a.isInstanceOf[UnitVector]) && (b.isInstanceOf[UnitVector]) && (a == b)) {Num(1)}
        else    {Dot(a,b)}
  ...
  {
    case Add(Num(d),v) => if (d == 0) {removeZeros(v)} else {Add(Num(d),removeZeros(v))}
    case Add(u,Num(d)) => if (d == 0) {removeZeros(u)} else Add(removeZeros(u),Num(d))
    case Add(u,v) => {Add(removeZeros(u),removeZeros(v))}
    ...
    def pullM(m:MExp) : MExp = m match
  {
    ...
    case MMul(m,u) =>
        if (nestedScalarsM(MMul(m,u))) {pullM(MMul(pullM(m), pullM(u)))}
        else {MMul(m,u)}
                           
    case SMMul(SMMul(a,b),v) => pullM(SMMul(a,b*v))
    case SMMul(u,v) => if (nestedScalarsM(u)) {pullM(SMMul(pullM(u),v))} else {SMMul(u,v)}
    case m:MExp => m
  }
  ...
  def pullV(v:VExp) : VExp = v match
  {
    ...
    case VMul(u,v) => if (nestedScalarsV(VMul(u,v))) {pullV(VMul(pullM(u),pullV(v)))} else {VMul(u,v)}
      
    case SMul(SMul(u,v),w) => pullV(SMul(u,v*w))
    case SMul(u,v) => if (nestedScalarsV(u)) {pullV(SMul(pullV(u),v))} else SMul(u,v)
    ...
    case Cross(SMul(u,v),a) => pullV(SMul(u x a,v))
        case Cross(a,b) => if (nestedScalarsV(Cross(a,b))) {pullV(Cross(pullV(a),pullV(b)))} else {Cross(a,b)}
  ...
  def pull(e:Exp) : Exp = e match
  {
     ...
     case Dot(u,v) => if (nestedScalars(Dot(u,v))) {pull(Dot(pullV(u),pullV(v)))} else {Dot(u,v)}
     case u:Exp => u
  }

...
object Variation
{
  ...
  def variationM( m:MExp ) : MExp = m match
  {
    ...
    case u:MExp =>
      if (u.isInstanceOf[ConstantMatrix]) {SMMul(u,Num(0))}
      else {deltaM(u)}
  }
  ...
  def variationV( v:VExp ) : VExp = v match
  {
    ...
    case VMul(a,b) =>
        if (a.isInstanceOf[ConstantMatrix]) {VMul(a,variationV(b))}
        else {VMul(variationM(a),b) + VMul(a,variationV(b))}
    case SMul(a,b) =>
        if (b.isInstanceOf[ConstantMatrix]) {SMul(variationV(a),b)}
        else {SMul(variationV(a),b) + SMul(a,variation(b))}
    case Cross(a,b) => Cross(variationV(a),b) + Cross(a,variationV(b))
    case v:VExp => if (v.isInstanceOf[ConstantVector]) {SMul(v,Num(0))} else {deltaV(v)}
  }
    ...
    def variationM_d( m:MExp ) : MExp = m match
  {
    ...
    case u:MExp =>
      if (u.isInstanceOf[ConstantMatrix]) {SMMul(desiredM(u),Num(0))}
      else {desiredM(u)}
  }
  ...
  def variationV_d( v:VExp ) : VExp = v match
  {
    ...
    case VMul(a,b) =>
        if (a.isInstanceOf[ConstantMatrix]) {VMul(desiredM(a),variationV(b))}
        else {VMul(variationM(a),desiredV(b)) + VMul(desiredM(a),variationV(b))}
    case SMul(a,b) =>
        if (b.isInstanceOf[ConstantMatrix]) {SMul(variationV_d(a),desired(b))}
        else {SMul(variationV_d(a),desired(b)) + SMul(desiredV(a),variation_d(b))}
    case Cross(a,b) => Cross(variationV_d(a),desiredV(b)) + Cross(desiredV(a),variationV_d(b))
    case v:VExp => if (v.isInstanceOf[ConstantVector]) {SMul(v,Num(0))} else {variationV(v)}
  }

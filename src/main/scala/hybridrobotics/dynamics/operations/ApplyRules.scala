...
object ApplyRules
{
    ...
    {
        case deltaV(u) => if (vrules.contains( deltaV(u))) {vrules(deltaV(u))}  else {deltaV(u)}
        case SMul(u,v) => SMul( applyConstraintsV(u, vrules, mrules), v)
        case VAdd(u,v) =>
            if (vrules.contains(VAdd( u, v))) {vrules(VAdd( u, v))}
            else {VAdd( applyConstraintsV(u, vrules, mrules), applyConstraintsV(v, vrules, mrules))}
        case Cross(u,v) =>
            if (vrules.contains(Cross( u, v))) {vrules(Cross( u, v))}
            else {Cross(applyConstraintsV(u, vrules, mrules), applyConstraintsV(v, vrules, mrules))}
        case VMul(a,b) => VMul(applyConstraintsM(a,mrules),applyConstraintsV(b,vrules, mrules))
        case v:VExp => if (vrules.contains(v)) {vrules(v)}  else {v}
    }
    ...
    def applyConstraints( e:Exp , rules:Map[Exp,Exp], vrules:Map[VExp,VExp] , mrules:Map[MExp,MExp]) : Exp = e match
    {
        ...
        case Dot(u,v) =>
            if (rules.contains(Dot(u,v))) {applyConstraints(rules(Dot(u,v)), rules, vrules, mrules)}
            else if (rules.contains(Dot(v,u))) {applyConstraints(rules(Dot(v,u)), rules, vrules, mrules)}
            else {Dot(applyConstraintsV(u,vrules,mrules),applyConstraintsV(v,vrules,mrules))}
        case u:Exp => u
    }
}
